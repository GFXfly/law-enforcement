...
- **听证权利专项复核**：根据当事人类型和罚款金额，自动判断是否需要告知听证权利（个人≥1万元，单位≥10万元），并检查告知内容是否规范。
...
根据《中华人民共和国行政处罚法》规定，系统现已支持听证权利的自动复核功能，确保行政处罚决定书符合法定程序要求。

### 法律依据

- **个人当事人**：罚款金额达到或超过 **1万元（10,000元）** 时，必须告知听证权利
- **单位当事人**（包括公司、企业、个体工商户等）：罚款金额达到或超过 **10万元（100,000元）** 时，必须告知听证权利
- 其他情形：责令停产停业、吊销许可证或执照等

### 实现方式

#### 1. 规则引擎检查（`administrative-penalty-rules.ts`）

新增以下函数：

- `extractFineAmount(text: string)`: 提取罚款金额，支持多种表述格式
  - "罚款300元"
  - "处罚款人民币300元"
  - "处以罚款3,000.00元"
  - 等各种变体

- `getPartyType(content: DocumentContent)`: 判断当事人类型
  - 通过关键词识别单位（公司、企业、商店、个体工商户等）
  - 通过身份证号识别个人
  - 返回 'individual' | 'unit' | 'unknown'

- `checkHearingRightRequired(content: DocumentContent)`: 综合检查
  - 识别当事人类型
  - 提取罚款金额
  - 应用不同标准判断是否需要听证
  - 返回详细的检查结果

#### 2. AI 语义分析（`ai-analysis-service.ts`）
...
3. 判断是否达到听证标准
4. 检查是否明确告知听证权利及申请期限
5. 如达到标准但未告知，报告为严重问题（critical）

AI 输出结构中新增 `hearingRightCheck` 字段：

```typescript
interface AIHearingRightCheck {
  partyType: 'individual' | 'unit' | 'unknown'
...
| 当事人类型 | 听证标准 | 告知要求 |
|----------|---------|---------|
| 个人 | ≥ 1万元 | 必须告知听证权利及3日申请期限 |
| 单位 | ≥ 10万元 | 必须告知听证权利及3日申请期限 |
| 未识别 | ≥ 1万元（保守策略） | 建议告知 |

### 问题报告

如果检测到听证权利告知缺失或不规范，系统将报告：

- **严重问题（critical）**：达到听证标准但未告知听证权利
- **警告问题（warning）**：已告知听证权利但未说明申请期限

### 测试用例

详见 `test-hearing-right.ts` 文件，包含5个测试场景：
1. 个人达到听证标准（1万元）
2. 个人未达到听证标准（5千元）
3. 单位达到听证标准（10万元）
4. 单位未达到听证标准（5万元）
...
规则检查会自动执行，无需额外配置。如果启用 AI 分析，系统会提供更详细的听证权利复核报告，包括：
- 当事人类型识别
- 罚款金额提取
- 听证权利要求判断
- 告知内容完整性检查
