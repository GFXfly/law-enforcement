              ...
              onClick={async () => {
                try {
                  ...
                  if (!response.ok) {
                    ...
                    throw new Error(error.error || "生成审查报告失败")
                  }

                  const blob = await response.blob()
                  const url = URL.createObjectURL(blob)
                  const link = document.createElement("a")
                  const baseName = results.files[0]?.name?.replace(/\.[^/.]+$/, "") || "文书"
                  link.href = url
                  link.download = `审查结果_${baseName}_${new Date().toISOString().slice(0, 10)}.docx`
                  document.body.appendChild(link)
                  link.click()
                  link.remove()
                  URL.revokeObjectURL(url)
                } catch (error) {
                  console.error("[Results] 导出报告失败:", error)
